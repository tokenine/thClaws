...
> These tables are **generated** from `model_catalogue.json`, the pricing SSOT.
> Don't edit them by hand — run `python3 scripts/gen-appendix-a.py` after a
> `make catalogue`, and commit both. `make appendix-a-check` fails the build
> if they drift.
...
Prices are **what you pay** — upstream cost with the 1.25× platform markup already applied.
...
Prices and context windows have different reliability, and the tables above
distinguish them:

| Field | How far to trust it |
|---|---|
| **Price** | Every row traces to a published source — the provider's own page or LiteLLM's feed. `make audit-pricing` fails a release if any priced row can't. |
| **Context without `?`** | Sourced: the provider's API, its docs, or LiteLLM. |
| **Context with `?`** | The provider's blanket default — a floor, not a specification. Roughly a fifth of gateway rows, concentrated in Qwen Cloud and DashScope, whose APIs don't return a window. |
| `openrouter/auto`, `openrouter/fusion*` | Pass-through — cost varies per request, so there is no canonical rate to print. |
